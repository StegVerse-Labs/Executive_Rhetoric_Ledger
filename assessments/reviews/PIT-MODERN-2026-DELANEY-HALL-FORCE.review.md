...
required_next_actions:
  ...
  - "Add remaining individualized event packets for arrests, crowd movement, and any verified baton strike or chemical-agent discharge."
  ...
  - "Supersede validation_results/delaney-hall-assessment-f7d1cc2e.pending.json only after a concrete validation result exists."
```
...
- cataloguing five individualized event packets while preserving direct-observation versus secondary-report distinctions.
...
Five individualized event packets are now catalogued:

1. ground restraint;
2. apparent chemical-agent posture;
3. baton display;
4. carry or guided removal;
5. reported observer or mediator chemical exposure.

The first four rely on direct visual observations from the supplied recording. The fifth relies on a secondary report and remains lower confidence pending primary media and official records.

No event packet establishes necessity, proportionality, lawfulness, intent, or constitutional justification.
...
The current pending receipt at `validation_results/delaney-hall-assessment-f7d1cc2e.pending.json` covers the five-event scope.

Earlier Delaney Hall validation receipts remain preserved as `superseded`. None received a concrete pass or failure result.

A future passing validation may establish only structural and cross-record consistency. It may not establish disputed facts, lawful detention or transfer authority, necessity, proportionality, constitutional justification, or liability.
...
4. event packets validate and remain no broader than their evidence;
5. an evidence reviewer confirms source roles and limitations;
6. a control reviewer confirms that missing controls remain visible and are not treated as completed;
